...
class TestSequenceFunctions(unittest.TestCase):
    ...
    def test_symbolic(self):
        R_nautical = (rotmat.R_s(2, "theta") * rotmat.R_s(1, "phi") *
                      rotmat.R_s(0, "psi"))
        print(R_nautical)
    ...
    def test_Euler(self):
        ...
        self.assertAlmostEqual(np.linalg.norm(correct -
                                              np.array(angles_euler)), 0)
    ...
    def test_dh(self):
        ...
        d = 5
        r = 10
        dh = np.array([[np.sqrt(2) / 2, -np.sqrt(2) / 2 * np.sqrt(2) / 2,
                        np.sqrt(2) / 2 * np.sqrt(2) / 2, 10 * np.sqrt(2) / 2],
        ...
        self.assertTrue(np.all(np.abs(dh - rotmat.dh(theta, d,
                                                     r, alpha)) < self.delta))
    ...
    def test_convert(self):
        result = rotmat.convert(quat.convert([0, 0, 0.1], to="rotmat"),
                                to="quat")

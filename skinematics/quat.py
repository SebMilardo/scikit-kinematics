...
Note that all the functions also work on arrays, and can deal with full
quaternions as well as with quaternion vectors.
...
class Quaternion():
    ...
    * access to the data, in the attribute *values*.
    ...
    inType : string
        ...
        following values "vector"[Default], "rotmat", "Fick", "Helmholtz",
        "Euler"
    ...
    export(to="rotmat")
        Export to one of the following formats: "rotmat", "Euler", "Fick",
        "Helmholtz"
    ...
    def __init__(self, inData, inType="vector"):
        ...
        if inType.lower() == "vector":
            if isinstance(inData, np.ndarray) or isinstance(inData, list):
                ...
                msg = ("Quaternions can only be based on ndarray or "
                       "Quaternions!")
                raise TypeError(msg)

        elif inType.lower() == "rotmat":
            """Conversion from rotation matrices to quaternions."""
            self.values = rotmat.convert(inData)
    ...
    def export(self, to="rotmat"):
        """
        Conversion to other formats. May be slow for "Fick", "Helmholtz",
        and "Euler".
        ...
        to : string
            ...
            * "rotmat": rotation matrices (default), each flattened to a
                        9-dim vector
            * "Euler" : Euler angles
            * "Fick" : Fick angles
        ...
        if to.lower() == "helmholtz":
            ...
            if rm.shape == (3, 3):
                rm = rm.reshape((1, 9))
            for ii in range(len(self)):
                hlmh = rotmat.rotmat2Helmholtz(rm[ii].reshape((3, 3)))
                Helmholtz[ii, :] = hlmh
            return Helmholtz
...
def deg2quat(inDeg):
    """
    Convert axis-angles or plain degree into the corresponding quaternion
    values.  Can be used with a plain number or with an axis angle.
...
def q_mult(p, q):
    """Quaternion multiplication: Calculates the product of two quaternions r
    = p * q If one of both of the quaterions have only three columns, the
    scalar component is calculated such that the length of the quaternion
    is one.  The lengths of the quaternions have to match, or one of the
    two quaternions has to have the length one.  If both p and q only have
    3 components, the returned quaternion also only has 3 components
    (i.e. the quaternion vector)
    ...
    if len(p) != len(q):
        amsg = ("Both arguments in the quaternion multiplication must have "
                "the same number of rows, unless one has only one row.")
        assert (len(p) == 1 or len(q) == 1), amsg
...
def quat2seq(quats, seq="nautical"):
    ...
    # If only the quaternion vector is entered, extend it to a full unit
    # quaternion
    if quats.shape[1] == 3:
        quats = unit_q(quats)
    ...
    if seq == "Fick" or seq == "nautical":
        ...
        # there the gamma-values are set to zero, since alpha/gamma is
        # degenerated
...
def calc_quat(omega, q0, rate, CStype):
    ...
    1) The output has the same length as the input. As a consequence, the
       last velocity vector is ignored.

    2) For angular velocity with respect to space ("sf"), the orientation
       is given by
    ...
    # The following is (approximately) the quaternion-equivalent of the
    # trapezoidal integration (cumtrapz)
    ...
    q_delta[omega_nonZero, :] = (omega_05[omega_nonZero, :] *
                                 np.tile(dq_total /
                                         omega_t[omega_nonZero],
                                         (3, 1)).T)
...
def calc_angvel(q, rate=1, winSize=5, order=2):
    ...
    dq_dt = signal.savgol_filter(q, window_length=winSize,
                                 polyorder=order, deriv=1,
                                 delta=1.0 / rate, axis=0)
...
if __name__ == "__main__":
    """Simple tests for proper output from functions

    More extensive tests in tests/test_quat.py"""

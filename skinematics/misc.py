...
def save_file(FilterSpec="*", DialogTitle="Save File: ", DefaultName=""):
    ...
    root = tkinter.Tk()
    root.withdraw()
    outFile = tkf.asksaveasfile(mode="w", title=DialogTitle,
                                initialfile=DefaultName,
                                filetypes=[("Save as", FilterSpec)])
    # Close the Tk-window manager again
    root.destroy()
...
if __name__ == "__main__":
    ...
    # (myFile, myPath) = get_file("*.eps", "Testing file-selection",
    #                             r"c:\temp\test.eps")
    # if myFile == 0:
    #     print(0)
    # else:
    #     print("File: %s, Path: %s" % (myFile, myPath))
    # (myFile, myPath) = save_file("*.txt", "Testing saving-selection",
    #                              r"c:\temp\test.txt")
    ...
    # ndList = [var for var in varList if
    #           type(curFrame.f_locals[var])==np.ndarray]
    # dfList = [var for var in varList if
    #           type(curFrame.f_locals[var])==pd.core.frame.DataFrame]
    # seriesList = [var for var in varList if
    #               type(curFrame.f_locals[var])==pd.core.series.Series]

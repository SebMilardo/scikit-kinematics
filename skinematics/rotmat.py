...
def stm_s(axis="x", angle="0", transl="0,0,0"):
    """Symbolic spatial transformation matrix about the given axis, by an
    angle with the given name, and translation by the given distances.
...
def R(axis="x", angle=90):
    ...
    if axis == "x":
        ...
        R = np.array([[ np.cos(a_rad), 0, np.sin(a_rad) ],  # noqa: E201 E202
                      [            0,  1,             0 ],  # noqa: E201 E202
                      [-np.sin(a_rad), 0, np.cos(a_rad) ]])  # noqa: E201 E202

    elif axis == "z":
        R = np.array([[np.cos(a_rad), -np.sin(a_rad), 0],
                      [np.sin(a_rad),  np.cos(a_rad), 0],
                      [            0,             0,  1]])  # noqa: E201 E202
...
def R_s(axis="x", angle="alpha"):
    """Symbolic rotation matrix about the given axis, by an angle with the
    given name
    ...
    if axis == "x":
        ...
        msg = '"axis" has to be "x", "y", or "z", not {0}'
        raise IOError(msg.format(axis))
    return R_s
...
def sequence(R, to="Euler"):
    ...
    if to == "Fick" or to == "nautical":
        ...
        epsilon = 1e-6
        beta = (-np.arcsin(np.sqrt(Rs[:, 0, 2] ** 2 + Rs[:, 1, 2] ** 2)) *
                np.sign(Rs[:, 1, 2]))
        ...
        # for the rest
        gamma[~small_indices] = (np.arcsin(Rs[~small_indices, 0, 2] /
                                           np.sin(beta)))
        alpha[~small_indices] = (np.arcsin(Rs[~small_indices, 2, 0] /
                                           np.sin(beta)))
...
def convert(rMat, to="quat"):
    ...
    # Catch small numerical inaccuracies, but produce an error for larger
    # problems
    epsilon = 1e-10
    if (np.min(np.vstack((1 + R11 - R22 - R33,
                          1 - R11 + R22 - R33,
                          1 - R11 - R22 + R33))) < -epsilon):
        raise ValueError('Problems with defintion of rotation matrices')

    q[1] = 0.5 * np.copysign(np.sqrt(np.abs(1 + R11 - R22 - R33)),
                             R32 - R23)
    q[2] = 0.5 * np.copysign(np.sqrt(np.abs(1 - R11 + R22 - R33)),
                             R13 - R31)
    q[3] = 0.5 * np.copysign(np.sqrt(np.abs(1 - R11 - R22 + R33)),
                             R21 - R12)
...
def seq2quat(rot_angles, seq="nautical"):
    """Given angles from sequential rotations, calculate corresponding
    quaternions.
    ...
    Notes
    -----
    The equations are longish, and can be found in 3D-Kinematics, 4.1.5

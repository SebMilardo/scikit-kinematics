...
magnetic field. The specifics of each sensor are hidden in the
sensor-object (specifically, in the "get_data" method which has to be
implemented once for each sensor). Initialization of a sensor object
includes a number of activities:

- Reading in the data.
- Making acceleration, angular velocity etc. accessible in a
...
class IMU_Base(metaclass=abc.ABCMeta):
    ...
    def get_data(self, in_file=None, in_data=None):
        """Retrieve "rate", "acc", "omega", "mag" from the input source and set the
        corresponding values of "self".  With some sensors, "rate" has to
        be provided, and is taken from "in_data".
    ...
    def __init__(self, in_file=None, q_type="analytical", R_init=np.eye(3),
        ...
        - Read in the data (which have to be either in "in_file" or in
          "in_data")
        - Make acceleration, angular_velocity etc. accessible, in a
          sensor-independent way
        - Calculates duration, totalSamples, etc
        - If q_type==None, data are only read in; otherwise, 3-D
          orientation is calculated with the method specified in "q_type",
          and stored in the property "quat".
        - If position==True, the method "calc_position" is automatically
        ...
        q_type : string
            ...
            - 'mahony' formula from Mahony, as implemented by
               Madgwick
        ...
        pos_init : (,3) vector
            initial position, currently only used in "analytical"
        in_data : dictionary
...
def analytical(R_initialOrientation=np.eye(3), omega=np.zeros((5, 3)),
    ...
    Do this using angular velocity and linear acceleration.
    ...
    if omega.ndim == 1:
        raise ValueError('Input to "analytical" requires matrix inputs.')
    ...
    for ii in range(accReSpace.shape[1]):
        vel[:, ii] = cumtrapz(accReSpace[:, ii], dx=1.0 / rate, initial=0)
        pos[:, ii] = cumtrapz(vel[:, ii], dx=1.0 / rate,
...
def kalman(rate, acc, omega, mag, D=[0.4, 0.4, 0.4],
    ...
    Q_k : None, or (7,7) ndarray
        covariance matrix of process noises. Parameter for tuning the filter.
        If set to "None", the defaults from Yun et al. are taken!
    R_k : None, or (7,7) ndarray
        covariance matrix of measurement noises. Parameter for tuning the
        filter; defaults from Yun et al.  If set to "None", the defaults
        from Yun et al. are taken!
    ...
    Based on "Design, Implementation, and Experimental Results of a
    Quaternion-Based Kalman Filter for Human Body Motion Tracking" Yun,
    X. and Bachman, E.R., IEEE TRANSACTIONS ON ROBOTICS, VOL. 22, 1216-1227
    (2006)
    ...
    # discrete state transition matrix Phi_k
    Phi_k = np.matrix(np.zeros((7, 7)))
    for ii in range(3):
    ...
    if Q_k is None:
        ...
        for ii in range(3):
            Q_k[ii, ii] = (D[ii] / (2 * tau[ii]) *
                           (1 - np.exp(-2 * tstep / tau[ii])))
    else:
    ...
    for ii in range(numData):
        ...
        basisVectors = np.vstack((magVec_n, np.cross(accelVec_n, magVec_n),
                                  accelVec_n)).T
        ...
        Phi_k[4, :] = np.r_[x_k[3] * tstep / 2, -x_k[6] * tstep / 2,
                            x_k[5] * tstep / 2, x_k[0] * tstep/2,
                            1, x_k[2] * tstep / 2, -x_k[1] * tstep / 2]
        ...
        # Projection of state
        # 1) quaternions
        x_k[3:] += tstep * 0.5 * quat.q_mult(x_k[3:],
                                             np.r_[0, x_k[:3]]).flatten()
...
class Mahony:
    """Madgwick's implementation of Mahony's AHRS algorithm
        ...
        Quaternion : output quaternion describing the Earth relative to the
                     sensor
